...
const Navbar = () => {
  ...
  return (
    <nav className="container flex flex-col md:flex-row relative mx-auto px-6 lg:px-0 my-6">
      ...
      <div className={ `${ state ? 'flex' : 'hidden' } md:flex flex-col md:flex-row justify-end items-center gap-3 md:gap-6 absolute md:relative left-0 right-0 bottom-0 bg-neutral-900 w-full md:mt-0 p-6 md:p-0 pb-3 md:pb-0 translate-y-full md:translate-y-0 z-10` }>
        <ul className='flex flex-col md:flex-row text-center w-full md:w-auto text-sm'>
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button className="w-full md:w-auto focus:outline-none focus:ring-4 focus:ring-neutral-50 rounded-lg text-xs md:text-sm text-neutral-500 hover:text-neutral-50 focus:text-neutral-50 font-semibold px-5 py-2.5">Login</button>
          <button className="w-full md:w-auto bg-neutral-50 hover:bg-neutral-200 focus:bg-neutral-200 focus:outline-none focus:ring-4 focus:ring-neutral-400 rounded-lg text-neutral-900 text-xs md:text-sm font-semibold px-5 py-2.5">Get Started</button>
        </div>
      </div>
    </nav>
  )
